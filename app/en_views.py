...
def home_view(request):
    return render(request, "en/app/home.html")


def about_view(request):
    return render(request, "en/app/about.html")


def experience_view(request):
    return render(request, "en/app/experience.html")


def licenses_view(request):
    return render(request, "en/app/licenses.html")


def services_view(request):
    return render(request, "en/app/services.html")


def action_aechanism_view(request):
    return render(request, "en/app/AechanismofAction.html")


def join_as_expert_view(request):
    return render(request, "en/app/JoinAsAnOfficer.html")
...
def join_as_expert_form_view(request):
    if request.method == "POST":
        ...
        if new_form.is_valid():
            ...
            email_body = render_to_string("en/component/join_email.html", context)

            subject = f"Request to Join as an Expert from: {name}"
            ...
            subject = "UHX"
            context = {}
            email_body = render_to_string("en/component/join_email_for_user.html", context)
            email = EmailMessage(subject, email_body, from_email, [to_email])
            email.content_subtype = "html"
            email.send()

            messages.success(request, "تم حفظ بياناتك بنجاح")
            return redirect("en_join_as_expert_officer")
        else:
            messages.error(request, "يوجد خطآ برجاء اعادة المحاولة")
            return redirect("en_join_as_expert_officer")
    return redirect(reverse("en_join_as_expert_officer"))


def vocation_request_view(request):
    return render(request, "en/app/consultation_request.html")
...
def vocation_request_form_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            email_body = render_to_string("en/component/vocation_email.html", context)

            subject = f"Request Consultation from: {first_name} {last_name}"
            from_email = settings.NOTIFI_EMAIL
            ...
            subject = "UHX"
            context = {}
            email_body = render_to_string("en/component/vocation_email_for_user.html", context)
            email = EmailMessage(subject, email_body, from_email, [to_email])
            email.content_subtype = "html"
            email.send()

            messages.success(request, "تم ارسال طلبك بنجاح")
            return redirect("en_consultation_request")
...
def vocation_request_form_from_home_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = "UHX"
            context = {}
            email_body = render_to_string("en_component/vocation_email_for_user.html", context)
            email = EmailMessage(subject, email_body, from_email, [to_email])
            email.content_subtype = "html"
            email.send()
